...
SEED = 42
IMAGE_ROOT='../images'
checkpoint_path = 'chkpt/saved.ckpt'
...
def model(inputs, num_classes=NUM_LABELS, is_training=True,
...
def findtruth(s,process_image,process_data_node,imno):
...
def printtruth(s,process_image,process_data_node,imno,fn):
  input_img,true_img = extract_data('jun004',imno)
  result_img = s.run(process_image,feed_dict={process_data_node: [input_img]})[0]
  mpi.imsave(fn,result_img)

proc,fd = inference()
sess = initSession(checkpoint_path)    
findtruth(sess,proc,fd,120)
printtruth(sess,proc,fd,120,"test.png")
