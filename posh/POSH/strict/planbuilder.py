...
class PlanBuilder:
    ...
    def build(self, agent):
        ...
        @rtype: L{POSH.strict.DriveCollection}
    ...
    def _buildDriveCollection(self, agent, competences, actionpatterns):
        ...
        @type competences: Dictionary, string -> L{POSH.strict.Competence}
        @param actionpatterns: An action pattern dictionary.
        @type actionpatterns: Dictionary, string -> L{POSH.strict.ActionPattern}
        ...
        for priority_element in self._drivecollection[3]:                        
            element_list = []            
            for element in priority_element[:len(priority_element)]:        
                drv_el=self._buildDriveElement(element, agent,competences, actionpatterns)
                drv_el.is_latched=False

                import POSH.latch
                for sense in drv_el._trigger._senses:
                    if issubclass(sense.behaviour.__class__, POSH.latch.Latch):
                        drv_el.is_latched=True
            
                element_list.append(drv_el)

            priority_elements.append(DrivePriorityElement(agent, dcname, element_list))
    ...
    def _buildDriveElement(self, element, agent,
        ...
        @rtype: L{POSH.strict.DriveElement}
        """
    ...
    def _buildCompetenceStubs(self, agent):
        ...
        @rtype: Dictionary, string -> L{POSH.strict.Competence}
        """
        stub_dict = {}
    ...
    def _buildActionPatternStubs(self, agent):
        ...
        @rtype: Dictionary, string -> L{POSH.strict.ActionPattern}
        """
        stub_dict = {}
    ...
    def _buildCompetences(self, agent, competences, actionpatterns):
        ...
        the corresponding L{POSH.strict.Action} objects are created. The
    ...
    def _buildActionPatterns(self, agent, competences, actionpatterns):
        ...
        actions / senses, the corresponding L{POSH.strict.Action} /
        L{POSH.strict.Sense} objects are created. The actions / senses have
    ...
    def _buildGoal(self, agent, goal):
        ...
        @rtype: L{POSH.strict.Trigger} or None.
        """
        if not goal:
            return None
        sense_list = []
    ...
    def _getTriggerable(self, agent, name,
        ...
        @type name: string
        @param competences: A competence object dictionary.
        @type competences: Dictionary, name -> L{POSH.strict.Competence}
        @param actionpatterns: An action pattern object dictionary.
        @type actionpatterns: Dictionary, name -> L{POSH.strict.ActionPattern}
        @return: The element with the given name.
        @rtype: L{POSH.strict.Action}, L{POSH.strict.Competence} or 
