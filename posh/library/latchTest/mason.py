...
a certain width and height, and is defined by a set of layered 'fields'.
...
agent to onesself) to a particular set of agents.
...
field. If an agent wants to find the closest male agent, it would
...
and then invoking the run() method of the created instance. Firstly, this causes
...
an agent's state within the environment have to be performed using the methods
of the MASON behaviour. The MASON behaviour also inherits the methods of the
L{OrientedEntity} and the L{Entity} class, and therefore also these methods.
...
class _MASONControl(Steppable):
    """The MASON interface that control the POSH agent.
    ...
    It provides a L{Steppable} MASON agent that gets its L{step()} method called
