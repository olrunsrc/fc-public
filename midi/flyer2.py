...
from steplist import Foot,Steplist
...
#    flyermmap = self.memmap
  ...
  def write_image(self,lock=True):
    scale = 1.0/2.0
    if (self.writecnt % 4) == 0: #TODO send every image
    #if (True):
	if lock:
	    self.sem.acquire()
	self.memmap.seek(28)
...
def main(args):
  ...
  try:
	...
	rospy.signal_shutdown("Esc Pressed")
      if key == ord('w'):
	mod.step_cam(50,0.01)
      if key == ord('a'):
	mod.step_cam(0.7,0.78)
      if key == ord('s'):
	mod.step_cam(0,0)
      if key == ord('d'):
	mod.step_cam(-0.7,-0.78)
      if key == ord('x'):
	mod.step_cam(50,-0.005)
      if key == ord('r'):
	mod.tilt_cam(0.17)
      if key == ord('f'):
	mod.tilt_cam(-0.17)
