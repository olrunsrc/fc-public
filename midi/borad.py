...
class Mstep(Mbase):
    yaml_tag = "!Mstep"
    Rval = [0.762,1.524,1.840,10.0,-10.0,-1.840,-1.524,-0.762]
    stride = [0.4,0.2,0.1,0.05,-0.1,0,0,0]
    def __init__(self,row,col):
        super(Mstep,self).__init__(row,col,'wlk')
        self.num = 0
        self.val = 0
        self.R = Mstep.Rval[col]
        self.t = Mstep.stride[row]/self.R
    def init(self): return True
    def noteon(self):
   	self.val = 127
        #print "(%d,%d) On" % (self.row,self.col)
...
class SmartPAD():
	...
	global theSmartie  #for the web page?
        theSmartie = self
        ...
        self.default_btns(self.ctl8x8, self.btn8x9, self.wlk8x8)
    ...
    def readin(self):
        ...
        while mi.poll():
            ...
            row = (msg[1] & 0xF0) >> 4
            col = msg[1] & 0x0F
            row2 = 20 - ((msg[1] & 0x7C) >> 2 ) #smartPAD mode 1 has
            col2 = msg[1] & 0x03                #2 columns of 4 buttons
            if row2 > 7:                        #80-83 to 52-55 in 1st col
                row2 -= 8                       #48-51 to 20-23 in 2nd col
                col2 += 4
            msgt = (cmd,chan,row2,col2)
            #print "Cmd %x Chan %x Row %x Col %x" % msgt
            if msgt == (0x8,2,7,7): print "Done"
            if msg[0]==130 and msg[1]==119:
                done = True
                break
            if msg[0]==176: #controller channel 1
            ...
            if msg[0]==128: #note off channel 1
                ...
                #break
            if msg[0]==145: #note on channel 2
                steps.append(self.wlk8x8[row2][col2].noteon())
                #break
            if msg[0]==129: #note off channel 2
                #steps.append(self.wlk8x8[row2][col2].noteoff())
                self.wlk8x8[row2][col2].noteoff()
                #break
            if msg[0]==146: #note on channel 3
                data.append(self.btn8x9[col][8].noteon())
                #break
            if msg[0]==130: #note off channel 3
                data.append(self.btn8x9[col][8].noteoff())
                #break
        return done, data, steps

    def loop(self):
        done, data, steps = self.readin()
        while not done:
            sleep(0.01)
            done, data, steps = self.readin()

    def default_btns(self,ctl8x8,btn8x9,wlk8x8):
        for i in range(7): #rows 0 - 6
            ctl8x8.append([])
            btn8x9.append([])
            wlk8x8.append([])
            for j in range(8): #cols 0 - 7
                ctl8x8[i].append( Mctl(self,i,j, 'm') )
                btn8x9[i].append( Mnote(i,j) )
                wlk8x8[i].append( Mstep(i,j) )
	    btn8x9[i].append( Mbtn(self,i,8) ) #col 8 of buttons
        ctl8x8.append([]) #row 7
        btn8x9.append([])
        wlk8x8.append([])
        for j in range(8): #cols 0 - 7 of row 7
            ctl8x8[7].append( Mctl(ctl8x8,7,j, 'y') )
            btn8x9[7].append( Mbtn(self,7,j) )
            wlk8x8[7].append( Mstep(7,j) )
        btn8x9[7].append( Mbtn(self,7,8) ) #the exit button col 8
        
        #specific replacement buttons
	#top row - left arm
	...
	#4th row - chest rpy, neck lowp midy uppp, pelvis ht, tim
	...
	ctl8x8[3][3] = Mctl(self,3,3,'npl',min=-0.1,max=2.0)
        ...
        ctl8x8[3][6] = Mctl(self,3,6,'pht',min=-0.4,max=0.3)
        ctl8x8[3][7] = Mctl(self,3,7,'tim',min=0.01,max=2.0)

	#5th row - torso-palm euclidian Lxyz Rxyz, ,tim
        ctl8x8[3][0] = Mctl(self,3,0,'lex',min=-3.0,max=3.0)
        ctl8x8[3][1] = Mctl(self,3,1,'ley',min=-3.0,max=3.0)
        ctl8x8[3][2] = Mctl(self,3,2,'lez',min=-3.0,max=3.0)
	ctl8x8[3][3] = Mctl(self,3,3,'',min=-0.1,max=2.0)
        ctl8x8[3][4] = Mctl(self,3,4,'rex',min=-1.0,max=1.0)
        ctl8x8[3][5] = Mctl(self,3,5,'rey',min=-0.6,max=0.6)
        ctl8x8[3][6] = Mctl(self,3,6,'rez',min=-0.4,max=0.3)
        ctl8x8[3][7] = Mctl(self,3,7,'tim',min=0.01,max=2.0)
