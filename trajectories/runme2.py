...
def main():
  ...
  try:
    ...
    stepCounter = 0
    armsent = footsent = False
    armnote = footnote = 'Wait '
    while not rospy.is_shutdown():
      done, curtim = monitor()
      print(footnote,armnote)
      if curtim > 40.0 and not armsent:
        footStepMsg,footnote = createFootStepCmd()
        armMsg, armnote = createArmCmd()
        armTrajectoryPublisher.publish(armMsg)
        armsent = True
        print('\a')
      if curtim > 45.0 and not footsent:
...
def launchval():
  global cmdx,cmdy,cmdn,cmdd,cmds,cmde,cmdl
  vallaunch = 'roslaunch srcsim qual2.launch init:=true '
  vallaunch += (('extra_gazebo_args:=' + cmde) if cmde != '' else cmde)
  print(vallaunch)
